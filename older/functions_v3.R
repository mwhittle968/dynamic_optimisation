...
fitness <- function(nh, ns, t){
  if (t%%2 != 0 | nh < nh.critical | ns < ns.critical){
      W = 0
      }
    ...
    W.ns = ns. #1/(1+exp(ns.critical-ns.)) # logistic by ns
    W = W.nh*W.ns
    }
  return(W)
}
...
investment <- function(d, s){
  s. = s + (d-1)/(2*s)
  return(s.)
}

regulation <- function(d, s){
  s. = s - (d-N-1)/3
  return(s.)
}
...
new.state <- function(t, d, s, ns, nh){
  if (t%%2 != 0 | nh < nh.critical | ns < ns.critical){ # If it's a non-reproductive time step, or the reserves are below critical levels,
    new.ns = chop((ns - ms + production(s)), 0, ns.max) # no resources are passed to larva
    if (d == 1){ # If d = 1, there is no investment or regulation
    new.nh = chop((nh - mh + N - maintenance(s)), 0, nh.max)
    new.s = s
    } else if (d > 1 & d <= (N+1)){ # If d is between 2 and N+1, there is investment
      new.nh = chop((nh - mh + N - maintenance(s) - (d-1)), 0, nh.max)
      new.s = chop((investment(d,s)), 0, s.max)
    } else { # If d is greater than N+1, there is regulation
        new.nh = chop((nh - mh + N - maintenance(s) - (d-N-1)), 0 , nh.max)
        new.s = chop((regulation(d,s)), 0, s.max)
    }
    } else { # If it's a reproducing time step and the reserves are sufficient, half the reserves are passed to larva
      new.ns = chop((((ns - ms)/2) + production(s)), 0, ns.max)
     if (d == 1){
       new.nh = chop(((nh - mh)/2 + N - maintenance(s)), 0, nh.max)
       new.s = s
     } else if (d > 1 & d <= (N+1)){
       new.nh = chop(((nh - mh)/2 + N - maintenance(s) - (d-1)), 0, nh.max)
       new.s = chop((investment(d,s)), 0, s.max)
     } else {
       new.nh = chop(((nh - mh)/2 + N - maintenance(s) - (d-N-1)), 0 , nh.max)
       new.s = chop((regulation(d,s)), 0, s.max)
     }
    }
